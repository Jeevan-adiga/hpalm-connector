...
public class DefaultTestCaseIdResolver implements TestCaseIdResolver {
	...
	@Override
	public Long getHpAlmTestId(ITestResult result) {
		String testClassName = null;

		if (result.getTestClass() != null) {
			Class<?> testClass = result.getTestClass().getClass();
	...
	@Override
	public Long getHpAlmTestConfigId(ITestResult result) {
		// try to find pattern
		Matcher m = PATTERN_CONFIG_ID.matcher(result.getName());
		if (m.find()) {
